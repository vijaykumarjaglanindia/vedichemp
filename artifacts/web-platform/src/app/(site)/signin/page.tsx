      ...
      <div id="phone" style={{ scrollMarginTop: 90, marginTop: "var(--sp-3)" }}>
        {otp !== "sent" || err === "otp-expired" ? (
          <form action={requestOtp} className="vh-card" style={{ display: "grid", gap: 12 }}>
            ...
            <div className="vh-row" style={{ gap: 14 }}>
              {(["BUYER", "SELLER"] as const).map((r) => (
                <label key={r} className="vh-row small" style={{ gap: 6, cursor: "pointer" }}>
                  <input type="radio" name="otprole" value={r} defaultChecked={r === "BUYER"} /> {r === "BUYER" ? "Buyer" : "Seller"}
                </label>
              ))}
            </div>
